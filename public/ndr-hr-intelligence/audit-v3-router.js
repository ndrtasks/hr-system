...
const V3='https://ecaexxjfzujoesptzurd.supabase.co/functions/v1/ndr-hr-audit-live-v3';
const previous=window.fetch.bind(window);
window.fetch=(input,init)=>{
  try{
    const url=typeof input==='string'?input:input?.url||'';
    if(url&&url.startsWith(V2)){
      const next=url.replace(V2,V3);
      if(typeof input==='string')return previous(next,init);
      return previous(new Request(next,input),init);
    }
  }catch(e){console.warn('NDR audit v3 router:',e)}
