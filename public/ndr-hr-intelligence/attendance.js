(()=>{
  ...
  async function loadRegister(force=false,silent=false){
    ...
    if(!force){const cached=readCache(month);if(cached){applyRegister(cached,month,true);setTimeout(()=>loadRegister(true,true).catch(()=>{}),80);return cached}}
  ...
  function bind(){
    ...
    q('attMonth').value=currentMonth();
    q('attReload').addEventListener('click',()=>{const m=q('attMonth').value||currentMonth();clearCache(m);loadRegister(true,false).catch(()=>{})});
    q('attMonth').addEventListener('change',()=>{register=null;loadedMonth='';loadRegister(false,false).catch(()=>{})});
    q('attSearch').addEventListener('input',applyFilters);q('attDepartment').addEventListener('change',applyFilters);q('attMode').addEventListener('change',applyFilters);q('attPrint').addEventListener('click',()=>window.print());q('attExport').addEventListener('click',exportExcel);q('attEditClose').addEventListener('click',closeEditor);q('attEditCancel').addEventListener('click',closeEditor);q('attEditBack').addEventListener('click',e=>{if(e.target===q('attEditBack'))closeEditor()});q('attEditSave').addEventListener('click',saveEditor);
    window.addEventListener('ndr:attendance-changed',()=>{const m=q('attMonth')?.value||currentMonth();clearCache(m);if(q('attendancePage')?.classList.contains('active'))loadRegister(true,true).catch(()=>{});else{register=null;loadedMonth=''}})
  }
