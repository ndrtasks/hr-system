(()=>{
  ...
  const LATE_GRACE=20, EARLY_GRACE=6;
  ...
  function setLoading(){q('attTableWrap').innerHTML='<div class="att-loading">يتم جمع الحضور والإجازات من Odoo…</div>'}
  ...
  function renderTable(){
    ...
    wrap.innerHTML=`<table class="att-table"><thead><tr><th>رقم الموظف</th><th>اسم الموظف</th><th>التاريخ</th><th>الحضور والانصراف</th><th>التأخير</th><th>الإجازة</th><th>الحالة</th><th>تعديل</th></tr></thead><tbody>${filtered.map((r,i)=>{const c=classification(r);return`<tr class="${c.problem?'needs-review':'normal-row'}"><td class="code">${esc(r.employeeCode||r.employeeId)}</td><td class="emp">${esc(r.employeeName)}</td><td><b>${esc(r.date)}</b><small>${esc(r.day)}</small></td><td class="punch">${esc(punchText(r))}</td><td class="late">${r.lateMinutes?`${esc(r.lateMinutes)} د`:'—'}</td><td class="leavecell">${esc(leaveText(r))}</td><td><span class="att-status ${c.cls}">${esc(c.label)}</span></td><td><button class="att-edit" data-att-edit="${i}" ${(!r.editable||r.status==='مستقبلي')?'disabled':''}>${r.attendanceId?'تعديل':'إضافة'}</button></td></tr>`}).join('')}</tbody></table>`;
    wrap.querySelectorAll('[data-att-edit]').forEach(b=>b.addEventListener('click',()=>openEditor(filtered[Number(b.dataset.attEdit)])));
  }
  async function loadRegister(force=false){const month=q('attMonth')?.value||currentMonth();if(!force&&register&&loadedMonth===month){applyFilters();return}setError('');setLoading();try{const range=rangeForMonth(month);register=await post('register',range);loadedMonth=month;populateDepartments();q('attPeriodLabel').textContent=`${range.from} — ${range.to}`;q('attGenerated').textContent=`آخر تحديث ${new Intl.DateTimeFormat('ar-SA',{timeZone:'Asia/Riyadh',hour:'2-digit',minute:'2-digit'}).format(new Date())}`;applyFilters()}catch(e){register=null;filtered=[];setError(e.message||String(e));q('attTableWrap').innerHTML=`<div class="att-empty"><b>تعذر تحميل الكشف</b><span>${esc(e.message||e)}</span><br><button id="attGoConnect" class="att-btn" style="margin-top:12px">فتح ربط Odoo</button></div>`;q('attGoConnect')?.addEventListener('click',()=>document.querySelector('.nav button[data-page="integrationPage"]')?.click())}}
  ...
  async function saveEditor(){if(!currentEdit)return;const cin=q('attEditIn').value,cout=q('attEditOut').value,reason=q('attEditReason').value.trim();if(!cin)return notify('وقت الدخول مطلوب');if(!reason)return notify('اكتب سبب التعديل');if(!confirm('سيتم حفظ التعديل مباشرة في Odoo. متابعة؟'))return;const btn=q('attEditSave');btn.disabled=true;btn.textContent='جار الحفظ…';try{const d=await post('save',{employeeId:currentEdit.employeeId,attendanceId:currentEdit.attendanceId||null,checkIn:`${currentEdit.date}T${cin}`,checkOut:cout?`${currentEdit.date}T${cout}`:'',reason});notify(d.message||'تم الحفظ في Odoo');closeEditor();await loadRegister(true)}catch(e){notify(e.message||String(e))}finally{btn.disabled=false;btn.textContent='حفظ في Odoo'}}
