...
import { Briefcase, LogIn, Users, ShieldCheck, ArrowRight, KeyRound, CheckCircle, Wifi, Loader2 } from 'lucide-react';
import { Role } from '../types';

type LoginView = 'SELECTION' | 'MANAGER' | 'EMPLOYEE';
...
const Login = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    const role: Role = view === 'MANAGER' ? 'MANAGER' : 'EMPLOYEE';
    
    try {
        const result = await loginWithCredentials(cleanEmail, cleanPassword, role);
        if (!result.success) {
  ...
  const renderSelection = () => (
    <div className="w-full max-w-4xl z-10 grid grid-cols-1 md:grid-cols-2 gap-6 animate-in zoom-in-95 duration-300">
        <button 
      ...
      <div className="w-full max-w-md z-10 animate-in slide-in-from-bottom-4 duration-300">
        <div className="bg-slate-900 border border-slate-800 p-8 rounded-2xl shadow-2xl relative">
            ...
            <div className="text-center mb-8">
                <div className={`w-14 h-14 mx-auto rounded-xl flex items-center justify-center mb-3 ${view === 'MANAGER' ? 'bg-amber-500/10 text-amber-500' : 'bg-blue-500/10 text-blue-500'}`}>
                    {view === 'MANAGER' ? <ShieldCheck size={28} /> : <Users size={28} />}
                </div>
                <h2 className="text-xl font-bold text-white">
                    {view === 'MANAGER' ? 'تسجيل دخول المدير' : 'تسجيل دخول الموظف'}
                </h2>
            ...
            {showRecovery ? (
                ...
                <form onSubmit={handleLogin} className="space-y-4">
                    ...
                    <button 
                        type="submit" 
                        disabled={isLoading}
                        className={`w-full font-bold py-3 rounded-lg mt-2 transition-all shadow-lg text-white flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed ${view === 'MANAGER' ? 'bg-amber-600 hover:bg-amber-700 hover:shadow-amber-500/20' : 'bg-blue-600 hover:bg-blue-700 hover:shadow-blue-500/20'}`}
                    >
                        {isLoading ? (
                            <>
                            ...
                            `دخول ${view === 'MANAGER' ? 'المدير' : 'الموظف'}`
                        )}
                    </button>
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-4 relative overflow-hidden font-cairo">
      <div className={`absolute top-0 right-0 w-96 h-96 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2 transition-colors duration-700 ${view === 'MANAGER' ? 'bg-amber-600/10' : 'bg-blue-600/10'}`}></div>
      <div className={`absolute bottom-0 left-0 w-96 h-96 rounded-full blur-3xl translate-y-1/2 -translate-x-1/2 transition-colors duration-700 ${view === 'MANAGER' ? 'bg-orange-600/10' : 'bg-emerald-600/10'}`}></div>
